...
function Dashboard({ pipelineKey, onLogout }: { pipelineKey: string; onLogout: () => void }) {
  ...
  async function handleScrape(type: "news" | "japanblog" | "topic") {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="text-xl font-bold text-gray-900">go2japan</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} disabled={generating}
                  ...
                  placeholder="e.g. Best street food in Tokyo" />
              </div>
              <div>
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-3">
              <button onClick={() => handleScrape("news")} disabled={scrapeLoading}
                ...
                {scrapeLoading && <Spinner />} Scrape Japan News
              </button>
              <button onClick={() => handleScrape("japanblog")} disabled={scrapeLoading}
                className="bg-indigo-600 text-white rounded-md px-4 py-2 text-sm font-medium hover:bg-indigo-700 disabled:opacity-60 disabled:cursor-not-allowed transition flex items-center gap-2">
                {scrapeLoading && <Spinner />} Scrape Japan Blog
              </button>
            </div>
...
const STORAGE_KEY = "go2japan_pipeline_key";
...
export default function PipelinePage() {
  ...
  return (
    <>
      <Head>
        <title>Pipeline Admin — go2japan</title>
