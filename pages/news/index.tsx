...
export default function NewsPage({ articles }: NewsPageProps) {
  ...
  const breadcrumbs = [
    { name: 'Home', href: '/' },
    { name: isNl ? 'Japan Nieuws' : 'Japan News', href: '/news/' },
  ];

  return (
    <>
      <SEOHead
        title={isNl ? "Japan Nieuws — Laatste Updates & Verhalen | Go2Japan" : "Japan News — Latest Updates & Stories | Go2Japan"}
        description={isNl ? "Blijf op de hoogte van het laatste Japan nieuws: toerisme-updates, visumwijzigingen, veiligheidswaarschuwingen en culturele verhalen. Dagelijks bijgewerkt." : "Stay informed with the latest Japan news: tourism updates, visa changes, safety alerts, and cultural stories. Updated daily."}
      />
        ...
        <section className="bg-white shadow-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <h1 className="text-4xl font-bold font-heading text-gray-900 mt-4">
              {isNl ? 'Japan Nieuws' : 'Japan News'}
            </h1>
            <p className="mt-3 text-lg text-gray-600 max-w-3xl">
              {isNl ? 'Het laatste nieuws uit Japan — toerisme-updates, visumwijzigingen, economie, veiligheid en cultuur. Dagelijks bijgewerkt uit betrouwbare bronnen.' : 'The latest news from Japan — tourism updates, visa changes, economy, safety, and culture. Updated daily from trusted sources.'}
            </p>
          </div>
        </section>
