...
const TRAVEL_NEWS_SOURCES = [
  "https://www.japantimes.co.jp/tag/travel/",
...
const JAPAN_BLOG_URL = "https://www.lonelyplanet.com/japan";
...
async function directFetch(url: string): Promise<string> {
  const response = await fetchWithTimeout(url, {
    headers: {
      "User-Agent": "Mozilla/5.0 (compatible; Go2JapanBot/1.0; +https://go2-japan.com)",
    },
  });
  if (!response.ok) {
...
export async function scrapeTravelNews(): Promise<ScrapedArticle[]> {
  const settled = await Promise.allSettled(
    TRAVEL_NEWS_SOURCES.map(async (sourceUrl) => {
      ...
      return {
        title: `Japan Travel News from ${hostname}`,
...
export async function scrapeJapanBlog(): Promise<ScrapedContent> {
  const content = await scrapeUrl(JAPAN_BLOG_URL);
  const titleMatch = content.match(/^#\s+(.+)/m);
  const title = titleMatch?.[1] || "Japan Travel — Latest News";
  return { url: JAPAN_BLOG_URL, content: content.slice(0, 6000), title };
}
...
export async function scrapeTopicContext(topic: string, priorityUrls?: string[]): Promise<string> {
  ...
  const [searchResults, detailedSearch, blogContent, newsContent, priorityData] = await Promise.allSettled([
    searchTopic(`${topic} Japan ${year} travel guide`),
    searchTopic(`${topic} Japan prices tips ${year}`),
    scrapeJapanBlog(),
  ...
  if (blogContent.status === "fulfilled") {
    parts.push(`JAPAN TRAVEL INFO — Editorial source:\n${blogContent.value.content}`);
  } else {
    console.warn("[scraper] Japan blog scrape failed:", blogContent.reason);
  }
