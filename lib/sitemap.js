...
const SITE_URL = 'https://go2-japan.com';
const DESTINATION_SLUG = 'japan'; // Used for /thailand-in/january/ style pages
const LOCALES = ['en'];
...
function getBlogSlugs() {
  try {
    // Check content/blog/en/ first (locale-based structure), then content/blog/
    const blogDirEn = path.join(contentDir, 'blog', 'en');
    const blogDirFlat = path.join(contentDir, 'blog');
    const blogDir = fs.existsSync(blogDirEn) ? blogDirEn : blogDirFlat;
...
function collectAllUrls() {
  ...
  if (citySlugs.length > 0) {
    ...
    citySlugs.forEach(slug => {
      ...
      // Note: city subpages (/food/, /hotels/, /attractions/, /weather/) are not implemented yet
    });
  }
  ...
  // Note: /islands/ route does not exist yet
  ...
  // Note: /transport/[slug]/ route does not exist yet
  ...
  // --- Weather ---
  // Note: /japan-in/[month]/ and /city/[slug]/weather/[month]/ routes do not exist
  urls.push({ path: '/weather/', priority: '0.9', changefreq: 'weekly' });

  // Note: /top-10/ and /compare/ routes do not exist yet
  ...
  urls.push({ path: '/tokyo-skytree-tickets/', priority: '0.9', changefreq: 'monthly' });
  urls.push({ path: '/mt-fuji-day-tour/', priority: '0.9', changefreq: 'monthly' });
  urls.push({ path: '/universal-studios-japan-tickets/', priority: '0.9', changefreq: 'monthly' });

  // --- E-E-A-T Trust pages ---
  urls.push({ path: '/about/', priority: '0.5', changefreq: 'monthly' });
  urls.push({ path: '/contact/', priority: '0.5', changefreq: 'monthly' });
  urls.push({ path: '/editorial-policy/', priority: '0.4', changefreq: 'yearly' });
  urls.push({ path: '/how-we-research/', priority: '0.4', changefreq: 'yearly' });
  urls.push({ path: '/affiliate-disclosure/', priority: '0.4', changefreq: 'yearly' });
  urls.push({ path: '/privacy/', priority: '0.3', changefreq: 'yearly' });
  urls.push({ path: '/terms/', priority: '0.3', changefreq: 'yearly' });
