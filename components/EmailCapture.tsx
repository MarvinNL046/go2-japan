...
export default function EmailCapture({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/subscribe', {
        ...
        body: JSON.stringify({ email, site: 'go2japan', locale: locale || 'en' }),
      });
  ...
  if (variant === 'sidebar') {
    return (
      <div className={`bg-brand-secondary text-white rounded-2xl p-6 ${className}`}>
        <span className="text-sm text-brand-primary font-medium">Stay in the loop</span>
  ...
  return (
    <div className={`bg-gradient-to-r from-brand-primary/5 to-brand-secondary/5 border border-brand-primary/15 rounded-2xl p-6 my-8 ${className}`}>
